...
LineC2::LineC2(int junction)
{
	if (junction == 1) {
		...
		id = 3;
        contour.push_back(cv::Point( 630, 90 ));
        contour.push_back(cv::Point( 720, 100));
...
void LineC2::drawRectangle(cv::Mat &imgFrame, cv::Scalar color){
...
bool LineC2::checkLineCrossed(std::vector<Blob> &blobs) {
	...
	for (auto blob : blobs) {
		...
		if (blob.blnStillBeingTracked == true && blob.centerPositions.size() >= 2) {
            ...
            if ((blob.centerPositions[prevFrameIndex].x > contour[0].x &&
                blob.centerPositions[currFrameIndex].x <= contour[1].x &&
                blob.centerPositions[prevFrameIndex].y > contour[0].y &&
                blob.centerPositions[currFrameIndex].y <= contour[1].y) ||
               (blob.centerPositions[prevFrameIndex].x > contour[2].x &&
                blob.centerPositions[currFrameIndex].x <= contour[1].x &&
                blob.centerPositions[prevFrameIndex].y > contour[1].y &&
                blob.centerPositions[currFrameIndex].y <= contour[2].y) ||
               (blob.centerPositions[prevFrameIndex].x > contour[3].x &&
                blob.centerPositions[currFrameIndex].x <= contour[2].x &&
                blob.centerPositions[prevFrameIndex].y > contour[3].y &&
                blob.centerPositions[currFrameIndex].y <= contour[2].y) ||
               (blob.centerPositions[prevFrameIndex].x > contour[3].x &&
                blob.centerPositions[currFrameIndex].x <= contour[0].x &&
                blob.centerPositions[prevFrameIndex].y > contour[0].y &&
                blob.centerPositions[currFrameIndex].y <= contour[3].y)) {
